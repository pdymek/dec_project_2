...
class SqlExtractConfig:
    ...
    def __init__(
        ...
        if extract_type not in SqlExtractConfig.EXTRACT_TYPES:
            raise Exception(f"Extract type '{extract_type}' is not supported. Please choose from {SqlExtractConfig.EXTRACT_TYPES}.")
        if source_table_name is None: 
            raise Exception(f"Please specify a source_table_name in your asset's config block.")
...
class DatabaseTableExtractor:
    ...
    def _full_extract(self) -> list[dict]:
        return self.source_postgresql_client.run_sql(self.sql_extract_parser.get_templated_sql(is_incremental=False))
    ...
    def _incremental_extract(self) -> list[dict]:
        if self.target_postgresql_client.table_exists(self.sql_extract_parser.config.source_table_name): 
            incremental_value = self._get_incremental_value()
            templated_sql = self.sql_extract_parser.get_templated_sql(is_incremental=True, incremental_value = incremental_value)
            return self.source_postgresql_client.run_sql(templated_sql)
        else: 
            logging.info(f"Table '{self.sql_extract_parser.config.source_table_name}' does not exist. Performing full extract.")
    ...
    def extract(self) -> list[dict]:
        ...
        if self.sql_extract_parser.config.extract_type == SqlExtractConfig.FULL_EXTRACT:
            ...
            raise Exception(f"Extraction type '{self.sql_extract_parser.config.extract_type}' is not supported. Skipping extraction.")

    def get_table_schema(self) -> tuple[Table, MetaData]: 
        return self.source_postgresql_client.get_table_schema(table_name=self.sql_extract_parser.config.source_table_name)
